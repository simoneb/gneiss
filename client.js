/*
Gneiss  - client spreadsheet editor. 
...
$(document).ready(function(e) {
	...
	var columnSFConstraints = {};	// sorting and filtering constraints for a column. index: col name. tied to column label's SF property
	...
	// start client spreadsheet by connecting to the server. port 8000 by default 
	...
	var spreadsheet_info={
		...
		title: "",
		cleanUp:"close",			// clean all the server data when closing the spreadsheet - default setting
		...
		if(columnInfo[col] != undefined){
			...
			if(columnInfo[col]["isStream"]){
				...
				if(streamFilters[rawURL] != undefined){
					if(streamFilters[rawURL]["filterBefore"] != undefined){
						$("#filter_stream").prop("checked", true);
						$("#filter_time_before").val(streamFilters[rawURL]["filterBefore"]);
						$("#filter_time_before").trigger("keyup");
					}
					if(streamFilters[rawURL]["filterAfter"] != undefined){
						$("#filter_stream").prop("checked", true);
						$("#filter_time_after").val(streamFilters[rawURL]["filterAfter"]);
						$("#filter_time_after").trigger("keyup");
					}
	...
	$("#filter_apply").click(function(e){
		...
		if($("#filter_stream").prop("checked")){
			...
			if(columnInfo[col] && columnInfo[col]["index"]){
				...
				if($("#filter_time_before_interp").prop("time") != undefined)
					streamFilters[rawURL]["filterBefore"]  = $("#filter_time_before_interp").prop("time");
				if($("#filter_time_after_interp").prop("time") != undefined)
					streamFilters[rawURL]["filterAfter"]  = $("#filter_time_after_interp").prop("time");
		...
		if(columnInfo[col] != undefined){
			if(columnInfo[col].source == "local"){				
				...
				if(columnInfo[col]["isStream"]){
					...
					console.log("here", streamFilters);
	...
	// LOADING SAVED FILE BACK HERE
	$("#chooseImport").change(function(e) {
       	// only reload when the selected file changes
		var reader = new FileReader();

      	// Closure to capture the file information.
      	reader.onload = function (e) {
            // read file, store objects
            output = "var obj="+e.target.result+";";
            //console.log("read file complete", output);
            try{
	            eval(output);
				console.log(obj);
			}
			catch(e){
				console.log("File format error - this is not a Gneiss file");
			}

			// first, need to clear EVERYTHING in the current editor
			// init all variables
			isSelect = false;			
			...
			// spreadsheet first
			Object.keys(cellConstraints).forEach(function(cell){
				cellConstraints[cell].set("");
				$(".cell_input[label='"+cell+"']").val("");
			});
			columnInfo = {};
			Object.keys(columnSFConstraints).forEach(function(col){
				columnSFConstraints[col].set("starting");
			});
			...
			// source pane
			$("#source_tab_ul").children(".source_tab").remove();
			$("#source_tab_ul").prepend('<li class="source_tab" id="source_tab1" name="1" style="background-color:white;" selected="selected">tab1</li>');			
			
			$("#url_bar").val("");
			urlBarConstraint.set("")
			Object.keys(webServiceConstraints).forEach(function(source){
				webServiceConstraints[source].set("");
			});

			$("#populated").prop("disabled", false).prop("checked", true);
			$("#populated_text").css("color", "black");
			...
			sourceTabs = [{type:"", path:""}];
			setSourceTabEventListeners($("#source_tab1"));
			webServiceConstraints = {};
			requestQueue = {};			
			streamFilters = {};
			isWebStreamingSource = {};

			localFiles = {};
			reStructuredDocs = [];

			// ui editor
			$("#web_tab_ul").children(".web_tab").remove();
			$("#web_tab_ul").prepend('<li id="webtab_index" class="web_tab" name="index" style="background-color:white" selected="true">index</li>');

			$("#editor_container").html('<div id="editor_index" name="index" class="web_editor_output" style="height:calc(100% - 10px); width:calc(100% - 15px); "><div id="element_hover_highlight" class="highlights"></div></div><div id="element_move_box" class="highlights"></div> <div id="element_move_line" style="position:absolute; display:none; width:100%; margin:0px;"><hr/><br/></div>');
			...
			uiConstraints = {};
			Object.keys(propTableConsraints).forEach(function(label){
				propTableConsraints[label].set("");
				$(".prop_input[label='"+label+"']").val("");
			});

			$("#mode_preview").trigger("click");
			$("#mode_edit").trigger("click");
...
var nowConstraint = cjs.constraint(getDateTime());
setInterval(function(){	
	nowConstraint.set(getDateTime());
}, 60000);

function NOW(){
	var time = nowConstraint.get();
...
function FETCHTIME(cell){
	...
	if(columnInfo[col] != undefined){
		...
		if(columnInfo[col]["source"] === "web" && columnInfo[col]["index"] != undefined && webServiceConstraints[columnInfo[col]["index"]] != undefined){
			var source = columnInfo[col]["index"];
